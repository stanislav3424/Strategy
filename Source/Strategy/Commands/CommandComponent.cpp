...
void UCommandComponent::BeginPlay()
{
    ...
    BehaviorTreeComponentRef = Owner->FindComponentByClass<UBehaviorTreeComponent>();
    if (!BehaviorTreeComponentRef)
         UE_LOG(LogTemp, Error, TEXT("UCommandComponent::BeginPlay - Owner %s does not have a BehaviorTreeComponent!"),
            *Owner->GetName());
